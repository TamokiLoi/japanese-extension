  ...
  return (
    <div className="mx-auto max-w-4xl px-4 py-6 md:px-8 md:py-10">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-2xl font-bold text-neutral-800">Từ vựng</h1>
          <p className="text-sm text-neutral-500">
            {isGrid ? `${list.length} thẻ` : `${list.length > 0 ? state.index + 1 : 0} / ${totalSelected}`}
          </p>
        </div>
      ...
      <div className="mt-4 flex flex-wrap items-center gap-2">
        <button
          ...
          className={`rounded-full border px-3 py-1 text-xs font-medium ${allChecked ? "border-rose-300 bg-rose-50 text-rose-600" : "border-neutral-200 text-neutral-500"}`}
        >
        ...
        {AVAILABLE_SOURCES.map((source) => {
          ...
          return (
            ...
            >
              {SOURCE_LABELS[source]} ({countForSource(source)})
            </button>
          );
        })}
        <select
